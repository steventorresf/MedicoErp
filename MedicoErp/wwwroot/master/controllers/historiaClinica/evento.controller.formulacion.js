(function () {
    ...
    AppController.$inject = ['$location', '$scope', '$cookies', 'ngDialog', 'HistClinicaEventosService', 'HistClinicaFormulacionesService', 'HistClinicaFormulacionesDetalleTempService', 'GenTablasDetalleService'];
    ...
    function AppController($location, $scope, $cookies, ngDialog, eveService, forService, fordettempService, tabdetService) {
        ...
        function guardar() {
            var data = {
                entityFor: vm.entityFor,
                idCentro: vm.userApp.idCentro,
            };

            var response = forService.create(data);
            response.then(
                function (response) {
                    regresar();
                },
